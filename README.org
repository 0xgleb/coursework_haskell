...
To compare Haskell to another high-level language (Ruby) in terms of the identified in analysis parameters of high-level languages I wrote several scripts.

** Sorting a big file

I chose this problem to compare performance of two high-level languages (Haskell and Ruby) and a low-level language (C). Even though this investigation is about high-level languages I've decided to include a C version of the script just to compare performance of high-level languages to low-level languages. Here is the algorithm that the scripts need to implement:

#+BEGIN_QUOTE
We want to read the file \texttt{"random\_numbers"}, which contains comma-separated integers, parse the contents to get the list of integers, sort them, convert back to the original format, and write the result to the file \texttt{"\{language name\}\_result"}.
#+END_QUOTE

Although the main purpose of this problem is to compare performance it also relates to the other criteria.

** Reverse polish server

This problem focuses mostly on safety, high-level abstractions and expressiveness of high-level languages. Here is the algorithm that the scripts need to implement:

#+BEGIN_QUOTE
We want to have a web server that has the following API endpoints:

POST: /check - extracts an expression in reverse polish notation and returns a response with code 200 and response body "valid" if the expression is valid or an response with error code 400 and error message "invalid" if the expression is invalid.

POST: /evaluate - extracts an expression in reverse polish notation and returns a response with code 200 and response body containing the result of evaluating the expression if the expression is valid, otherwise returns a response with error code 400 and error message "invalid".

We also want a client side that reads an expression in reverse polish notation from command line and then let's use pick an action: check or evaluate. If the use picks check then the script sends a /check request to the server and prints the result. If the user picks evaluate then the script sends an /evaluate request and prints the result.
#+END_QUOTE

This way I will show how high-level abstractions and types can be used for defining REST APIs and what the benefits of doing so.
...
For differentiating mathematical functions we can use dual numbers. In maths we define dual numbers this way:

#+BEGIN_CENTER
$x = a + b\epsilon$, where $\epsilon^2 = 0, \epsilon \neq 0$
#+END_CENTER

If we have a function $f$ that is defined for real numbers and we pass a dual number into it then we'll get this

$$ f(a + b\epsilon) = f(a) + bf'(a)\epsilon $$

This property is used for automatic differentiation.
