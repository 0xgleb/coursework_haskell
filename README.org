...
** Problematic Haskell fields
  ...
  birthdayUpdate :: User -> User
  birthdayUpdate usr = User (name usr) (age usr + 1)
  #+END_SRC

  As you can see we need to rewrite all the fields even if just one to change one of them. This is one of the problems with Haskell. To address this issue Haskell has a library called lens. They give us all the normal functionality of records and even more than that.
