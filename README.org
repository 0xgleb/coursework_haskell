...
[[./295429ede71982a0ce68544095ffed35.png]]
...
[[./bd71ca4f639ea360db4b9966446e5459.png]]
...
#+ATTR_LATEX: :width 7cm
[[./76b740316cb9f87f024dbe341cd65acc.png]]
...
#+ATTR_LATEX: :width 7cm
[[./1ecbb9b873d806a42ef7e5e42aa49a16.png]]
...
#+ATTR_LATEX: :width 7cm
[[./dc4eed15184fe1bc3325378d5c7a1706.png]]
...
One of classical examples of Haskell code, that shows how nice and expressive it is, is the Quicksort function.

#+BEGIN_SRC haskell
sort :: Ord a => [a] -> [a]
sort (x:xs) = qsort (filter (< x) xs) ++ [x] ++ qsort (filter (>= x) xs)
sort []     = []
#+END_SRC

At first glance it looks similar to the original Quicksort function, but it's actually less effective. It uses the same idea - divide and conquer, however the performance of the original Quicksort function relies on the very fast swap mechanism, which not something we can easily do in Haskell. As Haskell uses immutable data structures it doesn't swap any values in memory, it creates new ones.

Let's compare performance of Haskell's ~sort~ and C's ~qsort~. First I generated a big list of random numbers.
...
This code generates one million random numbers between $1$ and $1000$, then writes them into "random_numbers" file separated by commas.

Then I wrote some Haskell code to parse this file, sort the list, and write the result to another file.
