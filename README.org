...
Then I wrote some the code to parse this file, sort the list, and write the result to another file in Haskell, C, and Ruby.
...
Let's check if these scripts work.

[[./img/sort_test.png]]

Now let's change test files in the scripts to normal files, and change the number of numbers to a million.
